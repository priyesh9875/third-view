...
export class makeScene {
    ...
    animation( callbacks: animateCallback[], renderer: THREE.WebGLRenderer, scene: THREE.Scene, camera, stats?: Stats ) {
        /**
        ...
        */
        ...
        if( stats )  window.requestAnimationFrame( () => { this.animation( callbacks, renderer, scene, camera, stats ) } );
        else window.requestAnimationFrame( () => { this.animation( callbacks, renderer, scene, camera ) } );
