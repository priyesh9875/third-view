...
@Component( {
  ...
  templateUrl: 'partials/directional-light.html',
  providers: [ makeScene, Lights ]
} )

/**
 * This component shows behaviour of Light models.
 * Point light, directional light and ambient light are presented
...
export class DirectionalLightComponent implements OnInit {
    ...
    createScene() {
        ...
        /* Directional light */
        this.directionalLight = this.Lights.addDirectionalLight( this.scene, 0xffffff, 150, 250, 150 );
        folder = this.Lights.addDirectionalLightGUI( this.gui, this.directionalLight, this.aniCallback );
    ...
    toggle( target, what ) {
        ...
        switch( what ){
         ...
         case "ambient" :  if( ! this.ambientLight ) break;
          ...
          case "spot" :    let c = confirm( `SpotLight and directional lights cannot be seen properly together. 
                                             Do you want to navigate to spotLight page` );
                           if( c ) {
                               this.router.navigate( ['/LightType'] );
                           }
                           break;
                           
         case "directional" :  if( ! this.directionalLight ) break;
                           if( this.directionalLight.visible ) {
                               this.directionalLight.visible = false;
                               target.innerHTML = "Show directional light";
                               $( ".DirectionalLight" ).fadeOut();
                           } else {
                               this.directionalLight.visible = true;
                               target.innerHTML = "Hide directionalLight light";
                               $( ".DirectionalLight" ).fadeIn();
                           };
                           break;
                                                           
         default: console.log( "Ooppsssss" ); break;
